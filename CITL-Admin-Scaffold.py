...
def create_stack(path, stack_name):
    ...
    if stack_name == "STACK_SharePointAdmin":
        with open(os.path.join(stack_path, "src", "SharePointListProvisioner.cs"), "w", encoding="utf-8") as f:
            f.write("""
...
# Documentation files
with open(os.path.join(doc_root, "IntegrationMatrix.xlsx"), "w", encoding="utf-8") as f:
    f.write("")

with open(os.path.join(doc_root, "StackRoles.md"), "w", encoding="utf-8") as f:
    f.write("# Stack Roles\n\nDescribe ownership and responsibilities for each stack.")

with open(os.path.join(doc_root, "DevSetupInstructions.md"), "w", encoding="utf-8") as f:
    f.write("# Dev Setup Instructions\n\nDocument your environment, SDK installs, and dependencies here.")

print(f"[OK] Scaffold complete. Navigate to '{BASE_DIR}' to begin development.")
